...
st.set_page_config(page_title="Projektliniennetzplan V9.6", layout="wide")
st.title("✅ Projektliniennetzplan – V9.6 (Absturzfrei)")
...
if df[required_cols].isnull().any().any():
    st.error("❌ Leere oder ungültige Werte vorhanden.")
    st.stop()
...
if st.button("📈 Zeichnen & Exportieren"):
    try:
        ...
        for idx, row in df.iterrows():
            ...
            offset = offsets.get((d, row["Meilenstein"], linie), 0)
        ...
        st.download_button("⬇️ SVG herunterladen", data=svg_buf, file_name="netzplan_v9_6.svg", mime="image/svg+xml")
        st.download_button("⬇️ PDF herunterladen", data=pdf_buf, file_name="netzplan_v9_6.pdf", mime="application/pdf")
    except Exception as e:
